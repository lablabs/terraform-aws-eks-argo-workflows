...
To disable creation of IRSA server role and IRSA policy, set `server_irsa_role_create = false`.
...
| <a name="input_helm_chart_version"></a> [helm\_chart\_version](#input\_helm\_chart\_version) | Version of the Helm chart | `string` | `"0.20.8"` | no |
...
| <a name="input_helm_release_name"></a> [helm\_release\_name](#input\_helm\_release\_name) | Helm release name | `string` | `"argo-workflows"` | no |
...
| <a name="input_irsa_role_name_prefix"></a> [irsa\_role\_name\_prefix](#input\_irsa\_role\_name\_prefix) | The IRSA role name prefix for argo-workflows | `string` | `"argo-workflows-irsa"` | no |
| <a name="input_irsa_tags"></a> [irsa\_tags](#input\_irsa\_tags) | IRSA resources tags | `map(string)` | `{}` | no |
| <a name="input_namespace"></a> [namespace](#input\_namespace) | The K8s namespace in which the argo-workflows service account has been created | `string` | `"argo-workflows"` | no |
| <a name="input_server_irsa_additional_policies"></a> [server\_irsa\_additional\_policies](#input\_server\_irsa\_additional\_policies) | Map of the additional policies to be attached to server role. Where key is arbitrary id and value is policy arn | `map(string)` | `{}` | no |
| <a name="input_server_irsa_role_create"></a> [server\_irsa\_role\_create](#input\_server\_irsa\_role\_create) | Whether to create IRSA role and annotate service account for the server | `bool` | `true` | no |
| <a name="input_service_account_name_prefix"></a> [service\_account\_name\_prefix](#input\_service\_account\_name\_prefix) | The k8s argo-workflows service account name prefix. | `string` | `"argo-workflows"` | no |
| <a name="input_settings"></a> [settings](#input\_settings) | Additional helm sets which will be passed to the Helm chart values, see https://hub.helm.sh/charts/stable/argo-workflows | `map(any)` | `{}` | no |
| <a name="input_values"></a> [values](#input\_values) | Additional yaml encoded values which will be passed to the Helm chart, see https://hub.helm.sh/charts/stable/argo-workflows | `string` | `""` | no |
...
| <a name="output_server_iam_role_attributes"></a> [server\_iam\_role\_attributes](#output\_server\_iam\_role\_attributes) | Argo Workflows server IAM role attributes |
